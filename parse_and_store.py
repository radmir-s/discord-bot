...
def get_problem(amc="8", year=2014, problem_num=15):
    link = f"https://artofproblemsolving.com/wiki/index.php/{year}_AMC_{amc}_Problems/Problem_{problem_num}"
    ...
    while True:
        ...
        if isinstance(start, Tag):
            if start.has_attr("id"):
                if start["id"] == "Solution_1" or start["id"] == "Solution" or start["id"] == "toc":
                    break
    problem.insert(-1, "\\newline \\indent")
    return f"{year} AMC {amc} Problem {problem_num} \\newline \\indent " + "".join(
        problem) + "\\vspace{3mm}\n\n", images_links
